...
def msolve(m,z,r,M0,model,z_0):
    ...
    2)New model spatial mdot function for model=2.
    """
...
def rs_rhos_calc_new(M0, redshift=0.0, dz=0.1, zmax=7.0, N_ma=100, sigmalogc=0.128, N_herm=5, logmamin=-6, logmamax=None,sigmafac=0,N_hermNa=200,Namodel=3,N_r=5,mmodel=2,logr=True):

    """New modifications at rs_rhos_calc() function:
    1)Defining the number of position's values in the given range by N_r variable.
    2)There is the option of choosing distance/Rvir values in logspace by the argument logr=True.
    3)Defining the mass loss model with the option of new model (mmodel=2) and with no spatial information(mmodel=1).
    ...
    drshh = np.zeros((N_r,len(zdist),N_herm,len(ma200)))
    w_r = np.zeros((N_r,len(zdist),N_herm,len(ma200)))
    
    Oz_0=Omegaz(pOmega,redshift)
    Rvir = pow(3*M0*Msolar*pow(rhocrit0*g(redshift)*Delc(Oz_0-1)*4*np.pi,-1),1.0/3.0)
    m=np.zeros((N_r,N_ma))
    
    #Additional spatial condition on accreted subhaloes:
    def rmin_new(z,M=M0):
        slope = 0.004*np.log10(M)**3-0.091*np.log10(M)**2+0.791*np.log10(M)-1.036
        return 0.55*np.exp(-slope*z)
    ...
    for iz in range(len(zdist)):
        ...
        #Range of subhaloes position values in R_vir,host units:
        if mmodel==1:
            if logr==False:
                r_edges=np.linspace(0.01,1,N_r+1)
            else:
                r_edges=np.logspace(np.log10(0.01),0,N_r+1)
        else:
            if logr==False:
                if rmin_new(zdist[iz])>=0.01:
                    r_edges=np.linspace(np.log10(rmin_new(zdist[iz])),0,N_r+1)
                else:
                    r_edges=np.linspace(np.log10(0.01),0,N_r+1)
            else:
                if rmin_new(zdist[iz])>=0.01:
                    r_edges=np.logspace(np.log10(rmin_new(zdist[iz])),0,N_r+1)
                else:
                    r_edges=np.logspace(np.log10(0.01),0,N_r+1)
        ...
        for i in range(0,N_r):
            ...
            drshh[i,iz] = dr[i]*np.ones((N_herm,1))
            sol = odeint(msolve,ma,zcalc,args=(r[i],M0,mmodel,zdist[iz]))
            m0 = sol[-1]
        ...
        #Spatial weights of subhaloes which follow the host's NFW density profile.
        w_r[:,iz] = (radial_weights(rshh[:,iz],M=M0,z=redshift)*drshh[:,iz])/(sum(radial_weights(rshh[:,iz],M=M0,z=redshift)*drshh[:,iz]))
    ...
    if mmodel!=1:
        #Spatial probability w_r (the function radial_weights() is dN/dr).    
        #The total subhaloes weights are the product of Yang weights (accretion rate) and radial weights:
        weight=weight*w_r 
