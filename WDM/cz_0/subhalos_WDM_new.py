...
class subhalos:
    ...
    def __init__(self, mass_wdm=1.5):
        ...
        for ii in range(500):
            ...
            self.sum_rect  = Pk_interp(10**self.logk)*(10**self.logk)**2*10**self.logk*np.log(10) * (1+(self.a*(10**self.logk))**(2*1.12))**(-5./1.12)
            self.tot_sum   = self.tot_sum + self.sum_rect         
        self.log_k_min     = self.log_k_min - self.dlogk
        self.sum_rect_min  = Pk_interp(10**self.log_k_min)*(10**self.log_k_min)**2 * 10**self.log_k_min * np.log(10) * (1+(self.a*(10**self.log_k_min))**(2*1.12))**(-5./1.12)
        self.log_k_max     = self.log_k_max + self.dlogk
        self.sum_rect_max  = Pk_interp(10**self.log_k_max)*(10**self.log_k_max)**2 * 10**self.log_k_max * np.log(10) * (1+(self.a*(10**self.log_k_max))**(2*1.12))**(-5./1.12)
    ...
    #################################################################################
    def subhalo_distr(self,M0, accretion=False, redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0, N_hermNa=200, profile_change=True):
            
        ma200, z_a, rs_a, rhos_a, m0, rs0, rhos0, ct0, weight, survive = self.rs_rhos_calc(M0, redshift, dz, zmax, N_ma, sigmalogc,N_herm, logmamin, logmamax, sigmafac,N_hermNa, profile_change=True)
    ...
    #################################################################################
    def N_sat(self,M0, Mpeak=None,Mpeak_thres=False,redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0, N_hermNa=200, profile_change=True):
    ...
    #################################################################################
    def N_sat_Vthres(self,M0, Vpeak_max,Vpeak_thres=True,redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0, N_hermNa=200, profile_change=True):
...
########################################################################################################################### 
    ...
    def msolve(self,m,z,r,M0,model,N_hermNa):
        ...
        def mdot_r(r,z,M0,model):
            if model==2:
                ...
                if r>0.2:
                    ...
                    c=self.c_from_Mvir(M0*Msolar,z)
        ...
        if model==1:
            ...
            if z<0:
                z=0
            return mdot_r(r,z,M0,model)*AMz(z)*(m/tdynz(z))*pow(m/Mzvir(z),zetaMz(z))*pow(self.Hz(z)*(1+z),-1)
        
    
    
    def radial_weights(self,r,M,z):
        c=self.c_from_Mvir(Mvir=M*Msolar,z=z)
        M_all=np.log(1+c)-c/(1+c)
        Prob=(r*c**2)/((1+c*r)**2)
        return Prob/M_all

    
    def c_from_Mvir(self,Mvir,z):
    ...
    def rs_rhos_calc_new(self,M0, redshift=0.0, dz=0.1, zmax=7.0, N_ma=100, sigmalogc=0.128, N_herm=5, logmamin=-6, logmamax=None,sigmafac=0,N_hermNa=200,N_r=5,rmin=0.01,rmax=0.99,mmodel=3,NFW=True,logr=False, profile_change=True):
        ...
        for iz in range(len(zdist)):
            ...
            for i in range(0,N_r):
                ...
                sol = odeint(self.msolve,ma,zcalc,args=(r[i],M0,mmodel,N_hermNa))
                m0 = sol[-1]
                m0 = np.where(m0<10**(logmamin-1),10**(logmamin-1),m0)
                m[i,:]=m0            
            
                c200sub = self.conc200(ma200*Msolar,zdist[iz])
                rvirsub = pow(3*ma*Msolar*pow(rhocrit0*self.g(zdist[iz])*self.Delc(Oz-1)*4*np.pi,-1),1.0/3.0)
                r200sub = pow(3*ma200*Msolar*pow(rhocrit0*self.g(zdist[iz])*200*4*np.pi,-1),1.0/3.0)
                c_mz = c200sub*rvirsub/r200sub
                x1,w1 = hermgauss(N_herm)
                x1 = x1.reshape(np.alen(x1),1)
                w1 = w1.reshape(np.alen(w1),1)
                ...
                rhos_acc[i,iz] = ma*Msolar/(4*np.pi*rs_acc[i,iz]**3*self.fc(c_sub))
                ...
                if(profile_change==True):
                    ...
                    rs_z0[iz] = rs_acc[i,iz]
                    rhos_z0[iz] = rhos_acc[i,iz]
        ...
        Na = self.Na_calc(ma,zdist,M0,z0=0,N_herm=N_hermNa,Nrand=1000,sigmafac=sigmafac)
        Na_total = integrate.simps(integrate.simps(Na,x=np.log(ma)),x=np.log(1+zdist))
        weight = Na/(1.0+zdist.reshape(np.alen(zdist),1))
        weight = weight/np.sum(weight)*Na_total
        weight = (weight.reshape((len(zdist),1,len(ma))))*w1/np.sqrt(np.pi)
        ...
        #probability (the function radial_weights() is dN/dr):
            w_r = self.radial_weights(r,M=M0,z=redshift)*dr
            for i in range(0,len(w_r)):
                weight[i,:,:,:] = w_r[i]*weight[i,:,:,:]
            if (rmax-rmin)>0.9:
                weight = weight/w_r.sum()
    ...
    # Output: subhalo masses in units [M_solar], subhalo distribution dN/dm
    #################################################################################
    def subhalo_distr_new(self,M0, accretion=False, redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0,N_hermNa=200,profile_change=True,N_r=5,rmin=0.01,rmax=0.99,mmodel=3):
            
        ma200, z_a, rs_a, rhos_a, m0, rs0, rhos0, ct0, weight, survive ,pos= self.rs_rhos_calc_new(M0, redshift, dz, zmax, \
        N_ma, sigmalogc,N_herm, logmamin, logmamax, sigmafac,N_hermNa, profile_change=profile_change,                                                                                                   N_r=N_r,rmin=rmin,rmax=rmax,mmodel=mmodel)
    ...
    # Output: Total number of satellites, subhalo masses [M_solar], cumulative distribution subhalo mass
    #################################################################################
    def N_sat_new(self,M0, Mpeak=None,Mpeak_thres=False,redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0, N_hermNa=200, profile_change=True,N_r=5,rmin=0.01,rmax=0.99,mmodel=3):
    ...
    # Output: Total number of satellites, subhalo Vmax or Vpeak [km/s], cumulative distribution subhalo Vmax or Vpeak
    #################################################################################
    def N_sat_Vthres_new(self,M0, Vpeak_max,Vpeak_thres=True,redshift=0.0, dz=0.1, zmax=7.0, N_ma=500, sigmalogc=0.128, N_herm=5, logmamin=1, logmamax=None, sigmafac=0, N_hermNa=200, profile_change=True,N_r=5,rmin=0.01,rmax=0.99,mmodel=3):
