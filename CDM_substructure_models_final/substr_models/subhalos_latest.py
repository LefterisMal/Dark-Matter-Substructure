...
#I added these two functions for the c calculation from a given Mvir (for the dmdt(r) toy model relation)
...
def msolve(m,z,r,M0,model):
    ...
    4)Combination of toy model and new spatial fitting for 
...
def rs_rhos_calc_new(M0, redshift=0.0, dz=0.1, zmax=7.0, N_ma=100, sigmalogc=0.128, N_herm=5, logmamin=-6, logmamax=None,sigmafac=0,N_hermNa=200,Namodel=2,N_r=5,rmin=0.01,rmax=0.99,mmodel=3,NFW=True,logr=False):
    ...
    4)This function returns an additional array named rshh,which contains the positions of the subhalos in units of 
    distance/Rvir(host).
...
#Still work on progress the function below:
def rs_rhos_calc_alternative(M0, redshift=0.0, dz=0.1, zmax=7.0, N_ma=100, sigmalogc=0.128, N_herm=5, logmamin=-6, logmamax=None,sigmafac=0,N_hermNa=200,Namodel=2,N_r=5,mmodel=3,rmin=0.01,rmax=1,All=False,logr=False):
    
    length=int((zmax-redshift)/dz)*N_herm*len(ma200)*N_r
    ma200_matrix = np.zeros(length)
    m0_matrix = np.zeros(length)
    rs_acc = np.zeros(length)
    rhos_acc = np.zeros(length)
    rs_z0 = np.zeros(length)
    rhos_z0 = np.zeros(length)
    ct_z0 = np.zeros(length)
    survive = np.zeros(length)
    rshh = np.zeros(length)
    
    Na = Na_calc(ma,zdist,M0,z0=0,N_herm=N_hermNa,Nrand=1000,sigmafac=sigmafac,Namodel=Namodel)
    Na_total = integrate.simps(integrate.simps(Na,x=np.log(ma)),x=np.log(1+zdist))
    weight = Na/(1.0+zdist.reshape(np.alen(zdist),1))
    weight = weight/np.sum(weight)*Na_total
    weight = (weight.reshape((len(zdist),1,len(ma))))*w1/np.sqrt(np.pi)/N_r
    weight = np.array(N_r*[weight])
    
    if logr==False:
        r=np.linspace(rmin,rmax,N_r)
    else:
        r=np.logspace(np.log(rmin),np.log(rmax),N_r)
        
        
    for i in r:
        ma200_matrix[length*i:length*(i+1)],rs_acc[length*i:length*(i+1)],rhos_acc[length*i:length*(i+1)],m0_matrix[length*i:length*(i+1)],rs_z0[length*i:length*(i+1)],rhos_z0[length*i:length*(i+1)],ct_z0[length*i:length*(i+1)],weight[length*i:length*(i+1)],survive[length*i:length*(i+1)]=rs_rhos_calc_one_r(M0=M0, redshift=redshift, dz=dz, zmax=zmax, N_ma=N_ma, sigmalogc=sigmalogc, N_herm=N_herm, logmamin=logmamin, logmamax=logmamax,sigmafac=sigmafac,N_hermNa=N_hermNa,Namodel=Namodel,mmodel=mmodel,radius=i,All=False)
        
        #rshh=[length*i:length*(i+1)]=i
        weight[i,:,:,:] = w_r[i]*weight[i,:,:,:]
       
    
    weight = weight.reshape(length)    
    return ma200_matrix,m0_matrix,rs_acc,rhos_acc,rs_z0,rhos_z0,ct_z0,weight,survive,rshh
