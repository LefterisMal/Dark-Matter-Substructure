...
def radial_weights(r,M):
    ...
    """
    c=5   #This an extreme case of minimum concentration of a host halo.
    #c=c_from_Mvir(Mvir=M*Msolar,z=0)
    M_all=np.log(1+c)-c/(1+c)
    Prob=(r*c**2)/((1+c*r)**2)
    return Prob/M_all

def radial_weights2(r,c):
    M_all=np.log(1+c)-c/(1+c)
    Prob=(r*c**2)/((1+c*r)**2)
    return Prob/M_all

def radial_weights(r):
    #return radial_weights2(r,7)*np.exp(-7*r**2)/0.4011922661553602
    return radial_weights2(r,9)*np.exp(-7*r/2)/0.3367376864389688
'''

def radial_weights(x):
    #a=2.2
    #c=0.606
    #S=9.839/1.6385261053418323
    #return S*(201979*x-2000000*x**3)/(1009899*((250000*x**2/91809)+1)**(26/11))
    return -(2000000*x**3-2019798*x)/(1009899*((250000*x**2)/91809+1)**(26/11))/0.16653379473760027
